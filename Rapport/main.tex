...
Pour s'assurer que que la constante $c_O$ n'est pas présente dans le programme, on interdit aux fonction d'avoir un nom qui est préfixé apr c\_.
Chaque \verb|pap| c y est remplacé par \verb|pap| $c_O$ y. La fonction \textit{collect\_O} va s'assurer que les arguments soit owned.

La fonction $\delta_{\beta}$ va déduire la valeur de $\beta$ pour une fonction c. Pour réaliser cette étape, on commence par une approximation $\beta(c)$ = $\overline{B}$ , puis en calculant S = collect\_O(b). 
