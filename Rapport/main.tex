...
On introduit la fonction $\lambda_{reuse}$ qui se charge d'insérer les instructions \verb|inc| et \verb|dec| dans une fonction. Pour cela, elle fait appel à la fonction \textit{R} sur le corps de la fonction.
%La fonction \textit{R} utilise deux fonctions \textit{D} et \textit{S}.
...
\par La fonction \textit{D} cherche les variables mortes. Elle prend en paramètre la variable z à réutiliser et l'arité n du constructeur correspondant. 
...
Dans cette partie, on veut créer une map $\beta(c)$ qui associe à chaque fonction une liste qui donne le status des paramètres : Owned ou Borrowed.
...
\textit{collect\_O} qui retourne les variables qui ne doivent pas owned.

Pour déduire la valeur de $\beta(c)$ on commence par une approximation $\beta(c)$ = $\overline{B}$ , puis en calculant S = collect\_(b). Ensuite, nous mettons à jour  $\beta(c)_i :=$ O  si $y_i \in S $, et nous répétons ce processus jusqu'à atteindre un point fixe où aucune autre mise à jour n'est effectuée sur  $\beta(c)$. 
...
Noous allons maintenant tester notre interpréteur et notre compilateur sur la fonction de fibonacci.
...
\begin{table}[H]
	...
	\textbf{n} &
	  ...
	  \multicolumn{1}{c|}{\textbf{\begin{tabular}[c]{@{}c@{}}Temps\\ d'exécution\\ interprète\end{tabular}}} &
	  \multicolumn{1}{c|}{\textbf{\begin{tabular}[c]{@{}c@{}}Temps\\ d'exécution\\ compilé\end{tabular}}} &
	  \multicolumn{1}{c|}{\textbf{\begin{tabular}[c]{@{}c@{}}Allocations\\ interprète\end{tabular}}} &
	  \multicolumn{1}{c|}{\textbf{\begin{tabular}[c]{@{}c@{}}Allocations\\ compilé\end{tabular}}} &
	  \multicolumn{1}{c|}{\textbf{\begin{tabular}[c]{@{}c@{}}Objets\\ en vie\\ compilé\end{tabular}}} \\ \hline
	5  & 5       & 0 ms   & 0.016 ms  & 75         & 59         & 17        \\ \hline
	10 & 55      & 20 ms  & 0.044 ms  & 885        & 707        & 179       \\ \hline
	15 & 610     & 134 ms & 0.385 ms  & 9 865      & 7 891      & 1 975     \\ \hline
	20 & 6 765   & 1.74 s & 5.82 ms   & 109 455    & 87 563     & 21 893    \\ \hline
	25 & 75 025  & 23.3 s & 36.11 ms  & 1 213 925  & 971 139    & 242 787   \\ \hline
	30 & 832 040 & 5 mins & 371.63 ms & 13 462 685 & 10 770 147 & 2 692 539 \\ \hline
	\end{tabular}
...
(voir Figure~\ref{listing:fibo_naif}), l'on s'apperçoit que le code compilé prend moins de temps à s'exécuter que le code interprété,
mais surtout qu'il alloue moins d'espace mémoire pour s'exécuter. En effet, nous pouvons observer une réduction de 20\% du nombre 
d'allocations dans la mémoire. Cela est dû à l'insertion des instruction \verb|reset| et \verb|reuse|.
