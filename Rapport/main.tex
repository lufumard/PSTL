...
C'est aussi avec ce transformateur que l'on transforme les applications en applications partielles s'il n'y a pas assez d'arguments
...
Pour déduire la valeur de $\beta$ pour une fonction c, on commence par une approximation $\beta(c)$ = $\overline{B}$ , puis en calculant S = collect\_O(b). Ensuite, nous mettons à jour  $\beta(c)_i :=$ O  si $y_i \in S $, et nous répétons ce processus jusqu'à atteindre un point fixe où aucune autre mise à jour n'est effectuée sur $\beta(c)$. 
Une application ne doit pas avoir de paramètre borrowed. Pour résoudre ce problème, on définit un wrapper $c_O := c$. Dans notre implémentation, le nom de $c_O$ est de la forme c\_$nom de la fonction$. Qu'on ajoute dans $\beta$ et dont tous les paramètres sont owned. Chaque \verb|pap| c y est remplacé par \verb|pap| $c_O$ y.


\subsubsection{Insertion de inc et dec}
Dans cette partie, on insère les instructions \verb|inc| et \verb|dec|.
On suppose que notre map $\beta$ de la section précédente est bien définie. Pour chaque fonction c, on introduit une nouvelle map $\beta_l$ qui associe à chaque variable dans c, son status('B' ou 'O'). $\beta_l$ avec les valeurs de $\beta(c)$. Par défaut, une variable sera considérée comme owned.
...
Le \verb|dec| est un peu plus compliqué, car il ne suffit pas de décrémenter le nombre de références, il faut aussi 
...
(voir Figure~\ref{listing:fibo_naif}), l'on s'aperçoit que le code compilé est à beaucoup plus rapide que le code interprété,
