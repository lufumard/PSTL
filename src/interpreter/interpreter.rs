use crate::ast::{
    Const, Expr, Fn, FnBody, Var, AST, CONST_FALSE, CONST_LIST, CONST_NIL, CONST_TRUE,
};
...
impl Heap {
    pub(crate) fn get(&self, v: Var) -> Expr {
        let Var::Var(nom1) = &v;
        match self {
    ...
    pub(crate) fn add(&self, v: Var, val: Expr) -> Heap {
        return Heap::Some {
            var: v,
            value: val,
...
pub(crate) fn eval_cons_full_fn(
    args_fun: Vec<Var>,
    body_fun: FnBody,
    args: Vec<Var>,
    mut h: Heap,
